    ...
    <div class="mt-4 lg:mt-0 relative overflow-hidden">
        <div class="flex  justify-center w-full min-content-page-height">
            ...
            <div class="flex flex-col items-start xl:p-10">
                <h3 class="mb-3 text-5xl font-bold text-gray-700 montserrat-font">Hello, <br/>Welcome back</h3>
                @include('components.success_error')
                @include('auth.forgot_password_modal')

                <form action="{{ route('login.submit') }}" method="POST"
                      class="flex flex-col w-full h-full pb-6 mt-10 text-center bg-white rounded-3xl">
                    @csrf
                    ...
                    <div class="mb-2 text-left">
                        <label for="password" class="label-form">Mot de
                            passe @include('components.required')</label>
                        <div class="relative">
                        ...
                        <div class="flex justify-end mb-5">
                            <button type="button" id="forgot-password-button" class="text-green-700 text-sm mt-1">
                                Forgot password ?
                            </button>
                        </div>
                    ...
                    <p class="text-sm leading-relaxed text-left text-grey-900 z-50">Don't have a account ? <a
                            href="{{ route('register')}}" class="font-bold second-color mr-4">Register</a></p>
                </form>
            </div>
        </div>
        ...
        <img src="{{ asset('imgs/earth.png') }}"
             class="w-full top-[70%] absolute bottom-0 filter brightness-[85%] spin-clockwise z-0" alt="Image">
    </div>
